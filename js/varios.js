...
var sValorCal="";
				...
				if (isNaN(sValorCapturado))
					...
					if (nCapturado == (nTabla*i)){
						...
						//sValorCal=document.getElementById("Ok"+i).style="background-color: #4bf510 ";
							sValorCapturado = document.getElementById("txtResultado"+i).style="border: 4px solid #4bf510; background-color: #36b502";
					}
					else{
							if (nCapturado != (nTabla*i)){
								sValorCapturado = document.getElementById("txtResultado"+i).style="background-color: #e80808";
	...
	if (sErr == ""){
		document.getElementById("i").value=nCalif;
//	document.write("\n <p><lavel>"+"Calif = "+"<input type="+"text"+" value= " +nCalif+" /></lavel>"+"</p>");
}else
		alert(sErr);
}
